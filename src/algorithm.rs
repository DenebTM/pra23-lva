...
pub mod mfp {
    use crate::{
        analysis::{lv_entry, lv_entry_at, LVAnalysis, LVEntry, LVExit},
    ...
    pub fn run<'a>(program: &'a Program<'a>) -> LVAnalysis {
        ...
        let f_l = |p: &Program, a: &LVExit, l: Label| lv_entry_at(p, a, l);
        ...
        while work_list.len() > 0 {
            ...
            let (a, b) = (f_l(program, &ana, l), &ana[&l_p]);
            if !a.is_subset(&b) {
